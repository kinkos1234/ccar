...
function ActionButton({ children, color, ...props }: { children: React.ReactNode; color: string; [key: string]: unknown }) {
  const base = "px-4 py-2 rounded-lg text-[13px] font-medium transition-colors";
...
export default function CarEditPage() {
  ...
  function formatDateForInput(dateValue: unknown): string {
    ...
    if (typeof dateValue === 'string' && /^\d+$/.test(dateValue as string)) {
      dateValue = Number(dateValue);
    }

    // 문자열이 이미 YYYY-MM-DD 형태라면 그대로 사용
    if (typeof dateValue === 'string' && /^\d{4}-\d{2}-\d{2}/.test(dateValue as string)) {
      return (dateValue as string).slice(0, 10);
    }
      ...
      apiFetch(`/api/car/${id}`)
        .then((data: Record<string, unknown>) => {
          ...
          setForm({
            ...
            customerContactIds: ((data.customerContacts as {id: number}[] | undefined) || []).map((c) => String(c.id)),
          ...
          // Load internal contact users from mapping
          const contactUsers = (data.internalContactUsers as { id: number; name: string; department: string; email: string }[] | undefined) || [];
          const loadedContacts: InternalContact[] = contactUsers.map(u => ({
            type: "user" as const,
            userId: u.id,
            name: u.name,
            department: u.department,
          }));
          // If there's a text internalContact with manual names, add them
          const mappedNames = new Set(contactUsers.map(u => u.name));
          const textContact = (data.internalContact as string) || "";
          if (textContact) {
            const manualNames = textContact.split(/[,，]/).map(s => s.trim()).filter(Boolean).filter(n => !mappedNames.has(n));
  ...
  if (!isAuthChecked || loading) {
    return (
      <div className="p-8 bg-zinc-900 min-h-screen text-white text-center">
        <div className="flex flex-col items-center justify-center min-h-[50vh]">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#111167] mb-4"></div>
          {t('loading')}
        </div>
      </div>
    );
  ...
  if (!authenticated) {
    return (
      <div className="p-8 bg-zinc-900 min-h-screen text-white text-center">
        <div className="flex flex-col items-center justify-center min-h-[50vh]">
          <div className="bg-[#111113] rounded-lg p-8 max-w-md w-full border border-zinc-800/60">
  ...
  if (error) {
    return (
      <div className="p-8 bg-zinc-900 min-h-screen text-white text-center">
        <div className="flex flex-col items-center justify-center min-h-[50vh]">
          <div className="bg-[#111113] rounded-lg p-8 max-w-md w-full border border-zinc-800/60">
            <div className="text-red-400 text-lg mb-2">{t('errorPrefix')}</div>
  ...
  if (!form) {
    return (
      <div className="p-8 bg-zinc-900 min-h-screen text-white text-center">
        <div className="flex flex-col items-center justify-center min-h-[50vh]">
          <div className="text-zinc-400">{t('noCarInfo')}</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="p-8 bg-zinc-900 min-h-screen text-white">
      <div className="max-w-3xl mx-auto bg-[#111113] rounded-lg shadow p-8 border border-zinc-800/60">
        <h2 className="text-lg font-semibold text-zinc-100 mb-6">{t('editTitle')}</h2>
        <form onSubmit={handleSubmit}>
              ...
              {dropdownOpen && (
                <div className="absolute top-full left-0 w-full bg-[#111113] border border-zinc-800 rounded-lg shadow-2xl mt-1 max-h-60 overflow-y-auto z-10">
                  ...
                  <div className="border-t border-zinc-800">
                    <button
                      type="button"
                      className="w-full px-3 py-2 text-left hover:bg-zinc-800 text-[#111167] text-[13px] font-medium"
                      onClick={() => {
                  ...
                  <div className="p-6">
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <select
                          className={`${missingFields.includes('group') ? inputErrorClass : inputClass}`}
                        ...
                        {newCustomer.group === "__new" && (
                          <input
                            className={`mt-2 ${missingFields.includes('group') ? inputErrorClass : inputClass}`}
                      ...
                      <div>
                        ...
                        <select
                          className={`${missingFields.includes('department') ? inputErrorClass : inputClass}`}
                      ...
                      <div>
                        ...
                        <input
                          className={`${missingFields.includes('name') ? inputErrorClass : inputClass}`}
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-3 mb-4">
            <FormField label={t('corporation')}>
              <select
                ...
                onChange={handleChange}
                className={fieldError.corporation ? inputErrorClass : inputClass}
              >
            ...
            <FormField label={t('issueDate')}>
              <div className="relative">
                <input
                  ...
                  onChange={handleChange}
                  className={fieldError.issueDate ? dateInputErrorClass : dateInputClass}
                />
              </div>
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-3 mb-8">
            <FormField label={t('importance')}>
              <input
                ...
                onChange={handleChange}
                className={fieldError.importance ? inputErrorClass : inputClass}
                min={0}
                max={1}
                step={0.1}
                placeholder="0.0 ~ 1.0"
          ...
          <RiskCheckbox
            ...
            onRiskMitigationChange={(value) =>
              setForm(prev => prev ? ({ ...prev, riskMitigation: value }) : prev)
            }
            onRiskDescriptionChange={(value) =>
              setForm(prev => prev ? ({ ...prev, riskDescription: value }) : prev)
            }
            onRiskLevelChange={(value) =>
              setForm(prev => prev ? ({ ...prev, riskLevel: value }) : prev)
            }
            className="mt-6"
          />

          <div className="flex justify-between mt-8">
            <ActionButton color="gray" type="button" onClick={() => router.back()}>
              {t('cancel')}
            </ActionButton>
            <ActionButton color="blue" type="submit" disabled={loading}>
